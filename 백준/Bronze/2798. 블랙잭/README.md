...
### 메모
이것도 처음에는 조합으로 중복 안 되는 조합들을 뽑아서 계산하는 건가 생각했는데, 그러면 반대로 코드가 너무 복잡해지는 것 같음.
-> 그래서 그냥 바로 직전에 풀었던 2231. 분해합과 동일하게 브루트포스(노가다) 방식으로 풀기로 함.
a -> b = a+1부터 끝까지 -> c = b+1부터 끝까지 돌면서 max_num 단, 이 max_num은 M 이하여야 함.