...
각 행별로 최대값 받아서 이전까지의 최대값이랑 비교. 갱신.

근데 이것보다 더 쉽게 계산하는 방법이 있을 것 같음.
-> 다른 사람 코드 중에서 sum을 활용해서 2차원 리스트를 1차원으로 바꾸는 방법이 있었음.
그렇게 1행 99열짜리 리스트를 만든 뒤에, 최대값을 찾는다. 그 최대값의 index 값을 활용해서 //9 + 1와 %9 + 1를 활용해서 행, 열을 계산할 수 있었음
이것도 기발한 방법이다~
