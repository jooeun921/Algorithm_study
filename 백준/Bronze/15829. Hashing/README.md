...
### 메모
와 이거 문제 이해가 어려웠음. 근데 문제 이해만 하면 괜찮음!
아스키코드 활용해서 소문자 알파벳 리스트 생성.
input으로 받아온 string을 리스트 컴프리핸션으로 잘라서 알파벳 리스트에서 인덱스 찾아줌.
계수는 L 길이에 따라서 0부터 1씩 증가함.
그렇게 계산한 다음에 길이가 길 경우에 Mod M을 진행해서 줄임.

Mod는 해싱에서 데이터를 특정한 크기 내로 제한할 때 사용