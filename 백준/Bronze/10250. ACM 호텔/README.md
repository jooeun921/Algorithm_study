...
### 메모
 ...
 <p> 우선, 원하는 고객과 층수를 나눈 것의 나머지(몇바퀴 돌고 그 뒤에 어디에 멈추는지)가 층수라고 생각했는데, 생각해보니 나머지가 0일 때는 결국 최상층이라는 뜻이 되서 -> 지금은 우선 조건문으로 달아서 나머지가 0일때 층 floor는 최상층으로 할당함.
 다음에 호수 같은 경우에는, 몇바퀴 돌았는가의 문제라고 생각함 -> 몫에서 한번 더 돌아야 맞는 호수 -> 그러면 마찬가지로 나머지가 0인 경우에는 (층 6층, 손님 12번째) 602호여야 하는데 문제 발생. 따라서, 올림(math.ceil) 사용!
 마지막으로 호수 작성할 때, 101호, 103호처럼 0 처리를 위해서 str 덧셈 활용하기!
