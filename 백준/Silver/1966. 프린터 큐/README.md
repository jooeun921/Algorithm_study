...
1. 중요도가 높다 -> 숫자가 크다(처음에 숫자가 낮을수록 중요도가 높다고 생각했었음)
2. 리스트 배열을 옮기기 전의 초기 위치를 어떻게 저장해야 할까? -> enumerate() 함수 활용하기!

#### enumerate() 함수는 반복 가능한 객체(iterable)를 인덱스와 값을 함께 반환하는 함수.

start를 활용해 인덱스 시작 시점도 설정할 수 있음.

그렇게 한 뒤에는, 최대값을 갱신해가면서 최대값이 현재값보다 클 때마다 pop -> append를 통해 배열 이동.
