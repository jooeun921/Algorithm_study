...
각각의 테스트에 대해서 의상의 종류에 대해서 입을 수 있는 조합의 수를 계산하는 것.
모자 2개, 상의 3개 라고 했을 때 가능한 의상 조합은 2*3이지만, 안 입는다는 선택지도 있으므로, 3*4로 한 뒤에, 전부 입지 않는, 즉 알몸인 경우 1을 빼면 된다!
조합에 대한 수학 문제라고 볼 수 있음.
