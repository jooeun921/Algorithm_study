...
1. 가장 낮은 값과 가장 높은 값의 중간값을 가져온다.
2. count로 중간값으로 계산했을 때의 자른 랜선 수를 가져온다.
3. 자른 랜선 수가 잘라야 하는 N값보다 크면, low 값을 중간값을 높여 갱신한다. N값보다 작거나 같으면 중간값을 낮춘 뒤 high를 갱신한다.
4. low <= high, 즉 low가 high보다 커질 때까지 반복한다.
