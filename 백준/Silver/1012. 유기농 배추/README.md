...
BFS를 안다면 풀 수 있다!
그런데 이제 처음에 생각했을 때는 자기 지점 기준 상하좌우 한칸씩만 가능한 줄 알고, 횟수를 잘못 셌었는데, 그게 아니라 상하좌우에 붙어있기만 하면 가능했다
그래서 이제 현재 리스트 위치에서 값이 1이면, bfs 함수를 실행해서 상하좌우에 접근한 뒤에 1로 되어있는 값을 전부 0으로 초기화해주고 벌레 카운트를 1 높이면 됨.
