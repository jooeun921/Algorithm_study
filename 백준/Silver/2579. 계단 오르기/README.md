...
dp[i-1]로 i까지 도달했을 때의 최댓값을 구하는 것. dp에서 i-1로 하는 건, dp[0]이 계단이 하나일때의 점수이기 때문.
...
3부터는
1️⃣두계단 전에서 한칸 건너뛰고 오는 경우
2️⃣세계단 전에서 직전계단 갔다가 오는 경우
를 비교해서 max로 가져온다!
![alt text](image.png)
