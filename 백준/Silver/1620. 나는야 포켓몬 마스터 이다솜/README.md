...
포켓몬 이름을 조회하는 것은 그다지 어렵지 않다. 그러나 포켓몬 이름을 가지고 이게 몇번째 도감에 있는지 확인하는 건 리스트의 인덱스 조회이므로, 시간초과가 뜸.
따라서, 이걸 이름과 index 조합의 딕셔너리로 바꾸고 여기서 찾으면 시간복잡도 O(1)로 개선 가능.
