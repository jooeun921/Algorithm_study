...
- **문제 핵심**: 
  - 채팅방에서 발생한 기록을 처리하여 최종적으로 출력될 메시지를 생성하는 문제입니다.
  - 닉네임 변경 시, 이전 기록의 닉네임도 모두 변경되어야 합니다.

---

- **알고리즘**:
  1. **닉네임 관리**:
     - `id_name` 딕셔너리를 사용하여 각 유저 ID에 대한 최신 닉네임을 저장합니다.
     - `Enter`와 `Change` 명령어를 통해 닉네임을 갱신합니다.
  2. **명령어 저장**:
     - `command_list` 리스트에 `Enter`와 `Leave` 명령어를 저장합니다.
     - `Change` 명령어는 닉네임만 갱신하므로 출력 메시지에는 영향을 주지 않습니다.
  3. **출력 메시지 생성**:
     - `command_list`를 순회하며, `Enter`와 `Leave` 명령어에 따라 메시지를 생성합니다.
     - 메시지 생성 시, `id_name` 딕셔너리에서 최신 닉네임을 참조합니다.

---

- **시간 복잡도**:
  - `record` 순회: O(N) (N은 기록의 길이)
  - 메시지 생성: O(N)
  - 전체 시간 복잡도: **O(N)**
...
- **주의사항**:
  1. `Change` 명령어는 출력 메시지에 영향을 주지 않으므로, 닉네임만 갱신합니다.
  2. `Enter`와 `Leave` 명령어는 출력 메시지를 생성하므로, `command_list`에 저장합니다.
  3. 유저 ID는 고유하며, 닉네임은 중복될 수 있습니다.